...
def test_seed_creates_entities(test_db_stub):
    with test_db_stub.session() as session:
        repo = EnatRepository(session)
        events = repo.list_dirt_events()
        assert len(events) >= 1


def test_latest_metric_google(test_db_stub):
    with test_db_stub.session() as session:
        repo = EnatRepository(session)
...
def test_add_entity_cumulative(test_db_stub):
    with test_db_stub.session() as session:
        repo = EnatRepository(session)
...
def test_live_db_empty_without_stub(test_db):
