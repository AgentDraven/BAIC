...
describe("antigravity plugin", () => {
  ...
  it("emits provider-backed model utilization metrics", async () => {
    const ctx = makeCtx()
    ...
    const proMetric = result.metrics.find((metric) => metric.model === "Gemini Pro")
    expect(proMetric).toEqual(expect.objectContaining({
      key: "quota.model.utilization",
      unit: "ratio",
      source: "provider",
      confidence: 0.9,
    }))
    expect(proMetric.value).toBeCloseTo(0.2)
    expect(result.metrics.find((metric) => metric.model === "Gemini Flash").value).toBe(0)
  })
