(function () {
  ...
  var GOOGLE_CLIENT_ID = "<GOOGLE_OAUTH_CLIENT_ID>"
  var GOOGLE_CLIENT_SECRET = "<GOOGLE_OAUTH_CLIENT_SECRET>"
  ...
  function buildModelMetrics(configs) {
    var deduped = {}
    for (var i = 0; i < configs.length; i++) {
      var c = configs[i]
      var label = (typeof c.label === "string") ? c.label.trim() : ""
    ...
    for (var j = 0; j < keys.length; j++) {
      var model = deduped[keys[j]]
      var remaining = Math.max(0, Math.min(1, model.remainingFraction))
      metrics.push({
        key: "quota.model.utilization",
        value: 1 - remaining,
        unit: "ratio",
        model: model.label,
        source: "provider",
        confidence: 0.9,
        sourceUrl: "https://cloudcode-pa.googleapis.com",
        resetsAt: model.resetTime,
        periodDurationMs: QUOTA_PERIOD_MS,
      })
    }
    return metrics
  ...
  function probeAgyCloudCode(ctx, token) {
    ...
    return { plan: readAgyPlan(loadData), lines: lines, metrics: buildModelMetrics(parseAgyQuotaBuckets(quotaData)) }
  }
  ...
  function probeDiscovery(ctx, discovery) {
    ...
    return { plan: plan, lines: lines, metrics: buildModelMetrics(filtered) }
  }
  ...
  function probe(ctx) {
    ...
    if (ccData && !ccData._authFailed) {
      ...
      if (lines.length > 0) return { plan: null, lines: lines, metrics: buildModelMetrics(configs) }
    }
